...
export TOPG_GPU_MAX_USED_MB=1000
...
select_gpu_block() {
  ...
  # Filter by VRAM usage
  if mapfile -t _used < <(gpu_used_map); then
    declare -A used_mb
    for line in "${_used[@]}"; do
      IFS=',' read -r idx used <<< "$line"
      idx="$(echo "$idx" | sed 's/ //g')"
    ...
    for id in "${candidates[@]}"; do
      u="${used_mb[$id]:-999999}"
      if [ "$u" -le "$TOPG_GPU_MAX_USED_MB" ]; then
        filtered+=("$id")
      fi
    done
    candidates=("${filtered[@]}")
  else
    echo "warning: nvidia-smi not available; cannot filter by VRAM" >&2
  fi
...
if [ -n "${DEVICE}" ]; then
  ...
  if mapfile -t _used < <(gpu_used_map); then
    for line in "${_used[@]}"; do
      ...
      idx="$(echo "$idx" | sed 's/ //g')"
      used="$(echo "$used" | sed 's/ //g')"
      if [ "$idx" = "$DEVICE" ] && [ "$used" -gt "$TOPG_GPU_MAX_USED_MB" ]; then
        echo "GPU $DEVICE has ${used}MB used (limit ${TOPG_GPU_MAX_USED_MB}MB)" >&2
        exit 2
      fi
    done
  else
    echo "warning: nvidia-smi not available; cannot validate GPU usage" >&2
  fi
